...
void hkc_lock_physical_controls(homekit_characteristic_t* ch, const homekit_value_t value) {
    ch_group_t* ch_group = ch_group_find(ch);
    if (ch_group->main_enabled) {
        if (ch->value.int_value != value.int_value) {
            if (ch_group->homekit_enabled) {
            ...
            INFO("<%i> Lock Phy %i", ch_group->serv_index, value.int_value);
            ...
            do_actions(ch_group, 40 + ch->value.int_value);
        }
    }
...
void hkc_fan_rotation_speed(homekit_characteristic_t* ch, const homekit_value_t value) {
    ch_group_t* ch_group = ch_group_find(ch);
    if (ch_group->main_enabled) {
        if (ch->value.float_value != value.float_value) {
            if (ch_group->homekit_enabled) {
                led_blink(1);
            }
            
            INFO("<%i> Rot %g", ch_group->serv_index, value.float_value);
            
            ch->value.float_value = value.float_value;
            
            save_states_callback(ch_group);
            
            save_data_history(ch);
            
            do_wildcard_actions(ch_group, 4, ch->value.float_value);
        }
    }
...
void hkc_fan_swing_mode(homekit_characteristic_t* ch, const homekit_value_t value) {
    ch_group_t* ch_group = ch_group_find(ch);
    if (ch_group->main_enabled) {
        if (ch->value.int_value != value.int_value) {
            if (ch_group->homekit_enabled) {
            ...
            INFO("<%i> Swing %i", ch_group->serv_index, value.int_value);
            ...
            do_actions(ch_group, 42 + ch->value.int_value);
        }
    }
...
static void set_fx_color0(lightbulb_group_t* lightbulb_group) {
    ch_group_t* ch_group = ch_group_find(lightbulb_group->ch0);
    
    uint32_t current_color_ui32 = 0;
    
    uint16_t current_target[5];
    
    for (unsigned int i = 0; i < 5; i++) {
        current_target[i] = lightbulb_group->target[i];
    }
    
    hsi2rgbw(ch_group->ch[2]->value.float_value, ch_group->ch[3]->value.float_value, 100, ch_group);
    
    current_color_ui32 += ((uint8_t) (lightbulb_group->target[3] / 256)) << 24;
    current_color_ui32 += ((uint8_t) (lightbulb_group->target[0] / 256)) << 16;
    current_color_ui32 += ((uint8_t) (lightbulb_group->target[1] / 256)) << 8;
    current_color_ui32 += ((uint8_t) (lightbulb_group->target[2] / 256));
    
    for (unsigned int i = 0; i < 5; i++) {
        lightbulb_group->target[i] = current_target[i];
    }
    
    lightbulb_group->lightbulb_fx_data->colors[0] = current_color_ui32;
}
                    ...
                    if (LIGHTBULB_TYPE == LIGHTBULB_TYPE_NRZ && lightbulb_group->gpio[0] == addressled->gpio) {
                        ...
                        for (unsigned int p = LIGHTBULB_RANGE_START; p < LIGHTBULB_RANGE_END; p = p + LIGHTBULB_CHANNELS) {
                            for (unsigned int i = 0; i < LIGHTBULB_CHANNELS; i++) {
                                ...
                                if (lightbulb_fx_must_run) {
                                    //color = lightbulb_group->lightbulb_fx_data->leds_array[p - LIGHTBULB_RANGE_START + addressled->map[i]];
                                    
                                    const float color_full_brightness = lightbulb_group->lightbulb_fx_data->leds_array[p - LIGHTBULB_RANGE_START + addressled->map[i]];
                                    
                                    ch_group_t* ch_group = ch_group_find(lightbulb_group->ch0);
                                    const float color_dim_brightness = color_full_brightness * ch_group->ch[1]->value.int_value / 100;
                                    color = HAA_MIN(LIGHTBULB_MAX_POWER * color_dim_brightness, color_full_brightness);
                                    
                                } else {
                                    color = lightbulb_group->current[addressled->map[i]] >> 8;
                                }
                                ...
                                if (xSemaphoreTake(main_config.network_busy_mutex, MS_TO_TICKS(2000)) == pdTRUE) {
                                    ...
                                    if (action_network->method_n < 10) {
                                        ...
                                        if (action_network->method_n == 3) {        // TCP RAW
                                            ...
                                            action_network->len = strlen(method)
                                                + strlen(action_network->url)
                                                + strlen(http_header1)
                                                + strlen(action_network->host)
                                                + strlen(http_header2)
                                                + strlen(action_network->header)
                                                + ((method_req != NULL) ? strlen(method_req) : 0) + content_len_n
                                                + 4 + 1; // 4 for fixed chars of "%s /%s%s%s%s%s%s\r\n" +1 for last null only used for logs
                                        ...
                                        if (result >= 0) {
                                            if (action_network->method_n == 3 || action_network->method_n == 4) {
                                                ...
                                                INFO("<%i> Payload %i\n%s", ch_group->serv_index, action_network->len, req);
                                            }
                                        } else {
            ...
            if (xSemaphoreTake(main_config.network_busy_mutex, MS_TO_TICKS(2000)) == pdTRUE) {
                ...
                if (action_network->method_n < 10) {
                    ...
                    if (action_network->method_n > 0 &&
                        ...
                        if (action_network->method_n == 3) {
                            action_network->len = content_len_n + 1;    // +1 for last null only used for logs
                            
                            req = (char*) force_alloc(content_len_n);
                            if (!req) {
                                action_network->is_running = false;
                                action_network = action_network->next;
                                ERROR("DRAM");
                    ...
                    if (result >= 0) {
                        if (action_network->method_n == 4) {
                            ...
                            INFO("<%i> Payload %i\n%s", action_task->ch_group->serv_index, action_network->len, req);
                        }
...
void normal_mode_init() {
    ...
    float set_initial_state_data(ch_group_t* ch_group, const uint8_t ch_number, cJSON_rsf* json_context, const uint8_t json_index, const uint8_t ch_type, char** string_pointer) {
        ...
        switch (ch_type) {
            case CH_TYPE_INT8:
            case CH_TYPE_INT:
                state = ch_group->ch[ch_number]->value.int_value;
                break;
                
            case CH_TYPE_FLOAT:
                state = ch_group->ch[ch_number]->value.float_value;
                break;
                
            case CH_TYPE_BOOL:
                state = ch_group->ch[ch_number]->value.bool_value;
                break;
                
            default:    // case CH_TYPE_STRING
                break;
        }
        
        if (!json_context || cJSON_rsf_GetObjectItemCaseSensitive(json_context, INITIAL_STATE) != NULL) {
            unsigned int initial_state = 0;
            
            if (!json_context) {
                initial_state = json_index;
                
            } else {
                cJSON_rsf* json_initial_state = cJSON_rsf_GetObjectItemCaseSensitive(json_context, INITIAL_STATE);
                if (cJSON_rsf_IsArray(json_initial_state)) {
                    initial_state = (uint8_t) cJSON_rsf_GetArrayItem(json_initial_state, json_index)->valuefloat;
                } else {
    ...
    float set_initial_state(ch_group_t* ch_group, const uint8_t ch_number, cJSON_rsf* json_context, const uint8_t json_index, const uint8_t ch_type) {
        return set_initial_state_data(ch_group, ch_number, json_context, json_index, ch_type, NULL);
    }
...
#define UART_STOPBITS_0                     UART_STOP_BITS_MAX
    ...
    void new_accessory(const uint16_t accessory, uint16_t services, const bool homekit_enabled, cJSON_rsf* json_context) {
        ...
        if (cJSON_rsf_GetObjectItemCaseSensitive(json_context, HOMEKIT_NAME_SET) != NULL) {
            char* hk_accessory_name = uni_strdup(cJSON_rsf_GetObjectItemCaseSensitive(json_context, HOMEKIT_NAME_SET)->valuestring, &unistrings);
            accessories[accessory]->services[0]->characteristics[0] = NEW_HOMEKIT_CHARACTERISTIC(NAME_STATIC, hk_accessory_name);
        } else {
    ...
    void new_hk_ch_calloc(const uint16_t accessory, const uint16_t service, uint8_t ch_calloc, cJSON_rsf* json_context) {
        char* hk_service_name = NULL;
        if (cJSON_rsf_GetObjectItemCaseSensitive(json_context, HOMEKIT_NAME_SET) != NULL) {
            ch_calloc++;
            hk_service_name = uni_strdup(cJSON_rsf_GetObjectItemCaseSensitive(json_context, HOMEKIT_NAME_SET)->valuestring, &unistrings);
            INFO("Name \"%s\"", hk_service_name);
        }
        
        accessories[accessory]->services[service]->characteristics = calloc(ch_calloc, sizeof(homekit_characteristic_t*));
        
        if (hk_service_name) {
            accessories[accessory]->services[service]->characteristics[ch_calloc - 2] = NEW_HOMEKIT_CHARACTERISTIC(NAME_STATIC, hk_service_name);
        }
    }
    ...
    void new_switch(const uint16_t accessory, uint16_t service, const uint16_t total_services, cJSON_rsf* json_context, const uint8_t serv_type) {
        ...
        if (ch_group->homekit_enabled) {
            ...
            new_hk_ch_calloc(accessory, service, ch_calloc, json_context);
        ...
        if (max_duration > 0) {
            ...
            const uint32_t initial_time = (uint32_t) set_initial_state(ch_group, 1, NULL, 5, CH_TYPE_INT);
        ...
        if (get_initial_state(json_context) != INIT_STATE_FIXED_INPUT) {
            ...
            ch_group->ch[0]->value.bool_value = !((bool) set_initial_state(ch_group, 0, json_context, 0, CH_TYPE_BOOL));
            if (exec_actions_on_boot) {
                hkc_on_setter(ch_group->ch[0], HOMEKIT_BOOL(!ch_group->ch[0]->value.bool_value));
            } else {
                hkc_on_status_setter(ch_group->ch[0], HOMEKIT_BOOL(!ch_group->ch[0]->value.bool_value));
    ...
    void new_button_event(const uint16_t accessory, uint16_t service, const uint16_t total_services, cJSON_rsf* json_context, const uint8_t serv_type) {
        ...
        if (ch_group->homekit_enabled) {
            ...
            accessories[accessory]->services[service]->hidden = homekit_enabled - 1;
            accessories[accessory]->services[service]->primary = !(service - 1);
            new_hk_ch_calloc(accessory, service, 2, json_context);
            accessories[accessory]->services[service]->characteristics[0] = ch_group->ch[0];
    ...
    void new_lock(const uint16_t accessory, uint16_t service, const uint16_t total_services, cJSON_rsf* json_context) {
        ...
        if (ch_group->homekit_enabled) {
            ...
            new_hk_ch_calloc(accessory, service, 3, json_context);
            accessories[accessory]->services[service]->characteristics[0] = ch_group->ch[0];
            accessories[accessory]->services[service]->characteristics[1] = ch_group->ch[1];
        ...
        if (get_initial_state(json_context) != INIT_STATE_FIXED_INPUT) {
            ...
            ch_group->ch[1]->value.int_value = !((uint8_t) set_initial_state(ch_group, 1, json_context, 0, CH_TYPE_INT8));
            if (exec_actions_on_boot) {
                hkc_lock_setter(ch_group->ch[1], HOMEKIT_UINT8(!ch_group->ch[1]->value.int_value));
            } else {
                hkc_lock_status_setter(ch_group->ch[1], HOMEKIT_UINT8(!ch_group->ch[1]->value.int_value));
    ...
    void new_binary_sensor(const uint16_t accessory, uint16_t service, const uint16_t total_services, cJSON_rsf* json_context, uint8_t serv_type) {
        ...
        if (ch_group->homekit_enabled) {
            ...
            new_hk_ch_calloc(accessory, service, 2 + extra_data_size, json_context);
    ...
    void new_valve(const uint16_t accessory, uint16_t service, const uint16_t total_services, cJSON_rsf* json_context) {
        ...
        if (valve_max_duration > 0) {
            ...
            const uint32_t initial_time = (uint32_t) set_initial_state(ch_group, 2, NULL, 5, CH_TYPE_INT);
        ...
        if (get_initial_state(json_context) != INIT_STATE_FIXED_INPUT) {
            ...
            ch_group->ch[0]->value.int_value = !((uint8_t) set_initial_state(ch_group, 0, json_context, 0, CH_TYPE_INT8));
            if (exec_actions_on_boot) {
    ...
    void new_thermostat(const uint8_t accessory,  uint8_t service, const uint8_t total_services, cJSON_rsf* json_context, const uint8_t serv_type) {
        ch_group_t* ch_group = new_ch_group(10, 10, 6, 5);
        ...
        if (th_type != THERMOSTAT_TYPE_COOLER) {
            ...
            ch_group->ch[5]->value.float_value = set_initial_state(ch_group, 5, NULL, 5, CH_TYPE_FLOAT);
        }
        ...
        if (th_type != THERMOSTAT_TYPE_HEATER) {
            ...
            ch_group->ch[6]->value.float_value = set_initial_state(ch_group, 6, NULL, 5, CH_TYPE_FLOAT);
        }
        
        int lock_physical_controls = LOCK_PHYSICAL_CONTROLS_DEFAULT;
        if (cJSON_rsf_GetObjectItemCaseSensitive(json_context, LOCK_PHYSICAL_CONTROLS_SET) != NULL) {
            lock_physical_controls = (int) cJSON_rsf_GetObjectItemCaseSensitive(json_context, LOCK_PHYSICAL_CONTROLS_SET)->valuefloat;
            
            if (lock_physical_controls >= 0) {
                calloc_count++;
                ch_group->ch[7] = NEW_HOMEKIT_CHARACTERISTIC(LOCK_PHYSICAL_CONTROLS, 0, .setter_ex=hkc_lock_physical_controls);
                ch_group->ch[7]->value.int_value = set_initial_state(ch_group, 7, NULL, lock_physical_controls, CH_TYPE_INT);
            }
        }
        
        int fan_rotation_speed = FAN_ROTATION_SPEED_DEFAULT;
        if (cJSON_rsf_GetObjectItemCaseSensitive(json_context, FAN_ROTATION_SPEED_SET) != NULL) {
            fan_rotation_speed = (int) cJSON_rsf_GetObjectItemCaseSensitive(json_context, FAN_ROTATION_SPEED_SET)->valuefloat;
            
            if (fan_rotation_speed >= 0) {
                calloc_count++;
                ch_group->ch[8] = NEW_HOMEKIT_CHARACTERISTIC(ROTATION_SPEED, 0, .setter_ex=hkc_fan_rotation_speed);
                ch_group->ch[8]->value.int_value = set_initial_state(ch_group, 8, NULL, 5, CH_TYPE_FLOAT);
            }
        }
        
        int fan_swing_mode = FAN_SWING_MODE_DEFAULT;
        if (cJSON_rsf_GetObjectItemCaseSensitive(json_context, FAN_SWING_MODE_SET) != NULL) {
            fan_swing_mode = (int) cJSON_rsf_GetObjectItemCaseSensitive(json_context, FAN_SWING_MODE_SET)->valuefloat;
            
            if (fan_swing_mode >= 0) {
                calloc_count++;
                ch_group->ch[9] = NEW_HOMEKIT_CHARACTERISTIC(SWING_MODE, 0, .setter_ex=hkc_fan_swing_mode);
                ch_group->ch[9]->value.int_value = set_initial_state(ch_group, 9, NULL, fan_swing_mode, CH_TYPE_INT);
            }
        }
        ...
        if (ch_group->homekit_enabled) {
            ...
            if (lock_physical_controls >= 0) {
                accessories[accessory]->services[service]->characteristics[last_hk_ch] = ch_group->ch[7];
                last_hk_ch++;
            }
            
            if (fan_rotation_speed >= 0) {
                accessories[accessory]->services[service]->characteristics[last_hk_ch] = ch_group->ch[8];
                last_hk_ch++;
            }
            
            if (fan_swing_mode >= 0) {
                accessories[accessory]->services[service]->characteristics[last_hk_ch] = ch_group->ch[9];
            }
        }
        ...
        if (th_type >= THERMOSTAT_TYPE_HEATERCOOLER) {
            ...
            if (initial_target_mode == THERMOSTAT_INIT_TARGET_MODE_DEFAULT) {
                ch_group->ch[4]->value.int_value = set_initial_state(ch_group, 4, NULL, 5, CH_TYPE_INT8);
            } else {
        ...
        if (get_initial_state(json_context) != INIT_STATE_FIXED_INPUT) {
            ...
            ch_group->ch[2]->value.int_value = !((uint8_t) set_initial_state(ch_group, 2, json_context, 0, CH_TYPE_INT8));
    ...
    void new_lightbulb(const uint16_t accessory, uint16_t service, const uint16_t total_services, cJSON_rsf* json_context) {
        ch_group_t* ch_group = new_ch_group(10, 0, 2, 1);
    ...
    && !defined(CONFIG_IDF_TARGET_ESP32C61)
        ...
        if (LIGHTBULB_CHANNELS >= 3) {
            ...
            if (ch_group->homekit_enabled) {
                ...
                for (unsigned int s = 0; s <= 3; s++) {
                    accessories[accessory]->services[service]->characteristics[s] = ch_group->ch[s];
                }
            ...
            if (is_custom_initial)  {
                ...
                ch_group->ch[2]->value.int_value = set_initial_state(ch_group, 2, NULL, 5, CH_TYPE_INT);
            }
        ...
        if (is_custom_initial)  {
            ...
            ch_group->ch[1]->value.int_value = set_initial_state(ch_group, 1, NULL, 5, CH_TYPE_INT8);
        }
    ...
    void new_garage_door(const uint16_t accessory, uint16_t service, const uint16_t total_services, cJSON_rsf* json_context) {
        ...
        if (ch_group->homekit_enabled) {
            ...
            new_hk_ch_calloc(accessory, service, 4, json_context);
        ...
        GD_CURRENT_DOOR_STATE_INT = (uint8_t) set_initial_state(ch_group, 0, json_context, 0, CH_TYPE_INT8);
    ...
    void new_window_cover(const uint16_t accessory, uint16_t service, const uint16_t total_services, cJSON_rsf* json_context) {
        ...
        if (ch_group->homekit_enabled) {
            ...
            new_hk_ch_calloc(accessory, service, 5, json_context);
        ...
        WINDOW_COVER_HOMEKIT_POSITION = set_initial_state(ch_group, 0, NULL, 5, CH_TYPE_INT8);
    ...
    void new_sec_system(const uint16_t accessory, uint16_t service, const uint16_t total_services, cJSON_rsf* json_context) {
        ...
        SEC_SYSTEM_CH_CURRENT_STATE->value.int_value = set_initial_state(ch_group, 1, json_context, 0, CH_TYPE_INT8);
    ...
    void new_tv(const uint16_t accessory, uint16_t service, const uint16_t total_services, cJSON_rsf* json_context) {
        ...
        if (set_initial_state_data(ch_group, 1, NULL, 5, CH_TYPE_STRING, &configured_name) > 0) {
    ...
    void new_fan(const uint16_t accessory, uint16_t service, const uint16_t total_services, cJSON_rsf* json_context) {
        ...
        float saved_speed = set_initial_state(ch_group, 1, json_context, 0, CH_TYPE_FLOAT);
        ...
        if (get_initial_state(json_context) != INIT_STATE_FIXED_INPUT) {
            ...
            ch_group->ch[0]->value.bool_value = !((uint8_t) set_initial_state(ch_group, 0, json_context, 0, CH_TYPE_BOOL));
            if (exec_actions_on_boot) {
                 hkc_fan_setter(ch_group->ch[0], HOMEKIT_BOOL(!ch_group->ch[0]->value.bool_value));
            } else {
    ...
    void new_power_monitor(const uint16_t accessory, uint16_t service, const uint16_t total_services, cJSON_rsf* json_context) {
        ...
        if (ch_group->homekit_enabled) {
            ...
            new_hk_ch_calloc(accessory, service, 8, json_context);
            accessories[accessory]->services[service]->characteristics[0] = ch_group->ch[0];
            accessories[accessory]->services[service]->characteristics[1] = ch_group->ch[1];
            accessories[accessory]->services[service]->characteristics[2] = ch_group->ch[2];
        ...
        ch_group->ch[3]->value.float_value = set_initial_state(ch_group, 3, NULL, 5, CH_TYPE_FLOAT);
        ch_group->ch[4]->value.float_value = set_initial_state(ch_group, 4, NULL, 5, CH_TYPE_FLOAT);
        ch_group->ch[5]->value.int_value = set_initial_state(ch_group, 5, NULL, 5, CH_TYPE_INT);
    ...
    void new_free_monitor(const uint16_t accessory, uint16_t service, const uint16_t total_services, cJSON_rsf* json_context, const uint8_t serv_type) {
        ...
        ch_group->ch[0]->value.float_value = set_initial_state(ch_group, 0, json_context, 0, CH_TYPE_FLOAT);
    ...
    void new_data_history(const uint16_t accessory, uint16_t service, const uint16_t total_services, cJSON_rsf* json_context) {
        ...
        new_hk_ch_calloc(accessory, service, hist_size + 1, json_context);
